...
def build_category_cards(
    ...
    """Build a list of card components for all categories.
    ...
    category_history : dict[str, pd.Series]
        Full history per category (used for sparklines and deltas).

    Returns
    -------
    list
        List of Dash ``html.Div`` elements, one per category card.
    """
    ...
    for cat in CATEGORY_WEIGHTS:
        ...
        # Sparkline color: green if trending up today, red if trending down
        sparkline_color = COLORS["green"] if delta >= 0 else COLORS["red"]

        card = html.Div(
            className="metric-card",
            children=[
                # Header row: label + weight badge (no emojis)
                html.Div(
                    className="card-header",
                    children=[
                        html.Span(CATEGORY_LABELS[cat], className="card-label"),
                        html.Span(f"{weight_pct}%", className="card-weight"),
                    ],
                ),
                # Score + delta
                html.Div(
